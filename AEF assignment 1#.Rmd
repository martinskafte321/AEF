...
vw_momentum_portfolios = momentum_portfolios %>%
  ...
  summarize(excess_ret = weighted.mean(ret_excess, mktcap), .groups = "drop") # The value weighted return
...
The test statistics shows that is it possible to generate (very) small alpha by having a short position in low momentum stocks and a long position in high momentum stocks. The alpha of this regression is 0,012%, which may be considered as insignificant. The insignificant alpha can be implied by the graph above, which shows that alpha is highest within the lowest- and highest momentum portfolios. A strategy in this case would be to go long these portfolios and short the rest.
...
strategy_portfolio = momentum_portfolios %>% 
  group_by(permno) %>%
  mutate(ret_excess_k_month_ahead = lead(ret_excess,1)) %>%
  group_by(month) %>%
  mutate(breakpoint = median(momentum),
         portfolio = case_when(momentum <= breakpoint ~ "low",
                               momentum > breakpoint ~ "high")) 
