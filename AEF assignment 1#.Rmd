...
ret_excess_lag <- crsp_monthly %>%
  ...
  select(permno, month, ret_excess, ret_excess_lag) %>%
  drop_na()
...
all_guys <- new_momentum %>%
  ...
  group_by(month) %>%
  summarise(count_traded = sum(traded == 0) * 2) #CHECK THIS - we multiply number of trades by 2 because every transaction involves both buying and selling or selling and buying at the same time
...
all_guys_old <- old_momentum %>%
  ...
  mutate(traded = ifelse(portfolio==portfolio_lag, 1, 0)) %>%
  group_by(month) %>%
  summarise(count_traded = sum(traded == 0)) #CHECK THIS - we dont multiply by 2 because we assume just one trade per transaction, i.e. no stock goes from pf 10 to 1 or from 1 to 10 in one month
```
