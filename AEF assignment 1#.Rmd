...
# Briefly stating why the rpovided code provides incorrect results.
...
```{r}
ret_excess_lag %>%
  select(month, ret_excess_lag) %>%
pacf(month, ret_excess_lag)

pacf(ret_excess_lag[ret_excess_lag])
```
...
```{r}
#We dont use these lines for anything right now 
#cheaperportfolio = momentum_portfolios %>% 
  #mutate(portfolio = as.numeric(portfolio)) %>%
  #group_by(month) %>%
  #mutate(breakpoint = 5,
     #portfolio = case_when(portfolio <= breakpoint ~ "sell",
                               #portfolio >= breakpoint ~ "buy")) %>%
  #group_by(month, portfolio) %>%
  #summarise(excess_ret = weighted.mean(ret_excess, mktcap_lag)) # Value weighted return by high or low grouped momentum values. 

### Turnover for new portfolio
new_momentum = data_for_sorting %>%
  group_by(month) %>%
  mutate(
    portfolio = assign_portfolio( #Calling function from 4) to create 2 portfolios
    data = cur_data(), #use current set, i.e. assign_portfolios
      var = momentum,
      n_portfolios = 4
    ),
...
lag_guys <- new_momentum %>% 
  mutate(month = month %m+% months(1)) %>%
  select(permno, month, portfolio_lag = portfolio) %>%
  drop_na()
lag_guys

#Merging the lagged portfolio and calculate number of monthly trades
all_guys <- new_momentum %>%
  left_join(lag_guys, c("permno","month")) %>%
  drop_na() %>%
  mutate(portfolio = as.numeric(portfolio), portfolio_lag = as.numeric(portfolio_lag)) %>%
  select(permno, month, portfolio, portfolio_lag) %>%
  group_by(month) %>%
  mutate(trade = if (portfolio==1 && portfolio_lag ==2) {
                    1
                  } else if ( portfolio==1 && portfolio_lag ==3) {
                1
                  } else if ( portfolio==1 && portfolio_lag ==4) {
                2
                } else if ( portfolio==2 && portfolio_lag ==1) {
                1
                  } else if ( portfolio==2 && portfolio_lag ==4) {
                1
                  }
                else if ( portfolio==3 && portfolio_lag ==1) {
                1
                  } else if ( portfolio==3 && portfolio_lag ==4) {
                1
                } else if ( portfolio==4 && portfolio_lag ==1) {
                2
                  } else if ( portfolio==4 && portfolio_lag ==2) {
                1
                  }
                else if ( portfolio==4 && portfolio_lag ==3) {
                1
                } else 0
         ) %>%
  group_by(month) %>%
  summarise(count_traded = sum(trade != 0)) #CHECK THIS - we multiply number of trades by 2 because every transaction involves both buying and selling or selling and buying at the same time

count_doubletrades <- new_momentum %>%
  left_join(lag_guys, c("permno","month")) %>%
  drop_na() %>%
  select(permno, month, portfolio, portfolio_lag) %>%
  group_by(month) %>%
  mutate(trade = if (portfolio==1 && portfolio_lag ==4) {
                    1
                  } else if ( portfolio==4 && portfolio_lag ==1) {
                1
                } else 0) %>%
  summarise(count_doubles = sum(trade == 1)) %>%
  summarise(total = sum(count_doubles)) #We see that there is no observation in the entire data set where we have a stock going through 2 transactions from one month to the next (from 1 to 4 or 4 to 1)

#We dont use these lines for anything right now
#cheaperportfolio %>% 
  #pivot_wider(names_from = portfolio, values_from = excess_ret) %>%
  #mutate(high_low = buy - sell) %>% 
  #left_join(factors_ff_monthly, by = "month") %>%
  #lm(high_low ~ mkt_excess, data = .) %>% broom::tidy()


# Den her portefølje har vi allerede lavet. Den hedder momentum_portfolios
#######################################

### Turnover for old portfolio
old_momentum = data_for_sorting %>%
  group_by(month) %>%
  mutate(
...
#Creating a 1 month lag of the portfolio column so that we can compare from month to month 
lag_guys_old <- old_momentum %>%
  mutate(month = month %m+% months(1)) %>%
  select(permno, month, portfolio_lag = portfolio) %>%
  drop_na()

#Merging the lagged portfolio and calculate number of monthly trades
all_guys_old <- old_momentum %>%
  left_join(lag_guys_old, c("permno","month")) %>%
  drop_na() %>%
  mutate(portfolio = as.numeric(portfolio), portfolio_lag = as.numeric(portfolio_lag)) %>%
  group_by(month) %>%
  mutate(breakpoint_low = 1,
