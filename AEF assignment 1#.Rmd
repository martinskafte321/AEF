...
A portfolio that would capture a momentum effect but at the same time delivers lower trading costs, compared to the portfolio in 4, can be a portfolio with broader specifications of momentum. Instead of buying the upper 10th quantile of momentum and selling the 1st quantile, we buy the upper 3 quantiles and sell the lower 3. The idea is that this will result in a lower turnover, as assets enter and exit the buy and sell parts of the portfolio less frequently. 

```{r}
cheaperportfolio = momentum_portfolios %>% 
  mutate(portfolio = as.numeric(portfolio)) %>%
  group_by(month) %>%
  mutate(breakpoint_low = 3,
         breakpoint_high = 7,
         portfolio = case_when(portfolio <= breakpoint_low ~ "low",
                               portfolio >= breakpoint_high ~ "high")) %>%
  group_by(month,portfolio) %>%
