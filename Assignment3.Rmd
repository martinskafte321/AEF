...
First we calculate the return matric, where the entire series of returns for each stock is obtained. From this it is possible to calculate the historical mean (mu) and covariance matrix (sigma).
```{r}
...
The higher the level of transaction costs, the closer will the optimal portfolio be to the equal-weighted. Turnover penalization will make the investor reluctant towards making trades, as it is costly, and therefore choose appropriate portfolio-weights.
...
With rolling window forecasts, we recursively re estimate the model and store the one ahead covariance forecasts.  
...
performance_values <- list("MV (TC)" = performance_values,
                           ...
                           "MV (TC,reg)" = performance_values)

w_prev_1 <- w_prev_2 <- w_prev_3 <- w_prev_4 <- naive_weights # Initial weights of all portfolios
...
w_prev_2 = adjust_weights(w_MV_NSS, one_ahead_return)
  ...
  # Mean-variance transaction cost-adjusted portfolio with regular variance-covariance matrix:

  w_MV_TC_REG <- efficient_weights(mu = mu,
                                    sigma = sigma,
                                    lambda = lambda,
                                    w_prev = w_prev_4 # first naive, then adjusted. 
                                    )
  
  performance_values[[4]][p,] <- evaluate_performance(w = w_MV_TC_REG,
                                                      w_previous = w_prev_4,
                                                      next_return = one_ahead_return,
                                                      lambda = lambda
                                                      )
  
  w_prev_4 = adjust_weights(w_MV_TC_REG, one_ahead_return) # In the first period, this is the naive weight (1/N), but in the second and forward we adjust it. 
...
ret_matrix.xts <- xts(ret_matrix %>% select(-month), order.by = ret_matrix$month)

N <- ncol(ret_matrix.xts)

# Specify GARCH model:
spec <- ugarchspec(variance.model = list(model = 'sGARCH',
                                         garchOrder = c(1,1)))

# replicate the specification for each of our N assets:
...
apply(pred * ret_matrix.xts %>% tail(581), 1, sum) %>% 
  enframe() %>% 
