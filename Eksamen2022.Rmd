...
w_efficient_tbl %>% kbl(caption = "Table 4: Weights assigned to stocks based on the model implied excess returns", digits = 4) %>%
  kable_classic(full_width = F, html_font = "Cambria")
...
colnames(weights_all) <- c('permno','Exercise_1', 'Exercise_2', 'Naive', 'Efficient')
weights_all %>% kbl(caption = "Table 7: Weights from the 4 portfolios", digits = 4) %>%
  kable_classic(full_width = F, html_font = "Cambria")



portfolio_data %>% select(month, permno, ret_excess) %>% left_join(weights_all, by = 'permno') %>% mutate(naive_ret = Naive*ret_excess,
                                                                                                          exc1_ret = Exercise_1*ret_excess,
                                                                                                          excs2_ret = Exercise_2*ret_excess,
                                                                                                          efficient_ret = Efficient*ret_excess
                                                                                                          ) %>% group_by(permno) %>% summarise(naive = mean(naive_ret), naive_sd = sd(naive_ret), efficient = mean(efficient_ret), efficient_sd = sd(efficient_ret), exc1 = mean(exc1_ret), exc1_sd = sd(exc1_ret), exc2 = mean(excs2_ret), exc2_sd = sd(excs2_ret))
