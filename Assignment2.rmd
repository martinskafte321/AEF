...
Moreover, the following four macro indicators are included: dp, ep, bm and ntis, where dp is the dividend-price ratio which denotes a shares dividend as a percentage of its price, ep is the earnings-price ratio which shows a company's earnings per share relative to its price, bm is the book-to-market ratio, i.e. the value as per assets minus liabilities relative to the market value and ntis is the net equity expansion which denotes the ratio of twelve-month moving sums of net issues by listed
stocks to total end-of-year market capitalization of same listed stocks. All variables are deemed relevant indicators when one aims at either investigating the economic outlook of firms (dp, ntis) or taking the temperature of the market, i.e. measuring how aggressively investors approach investments (ep, bm). \
...
The book-to-market ratio measures the total book value (that is, company value from earnings statement) relative to the market value (on the stock market). The Net Equity Expansion measures the ratio of 12-month moving sums of net issues on the NYSE divided by the total end-of-year market cap of NYSE stocks. Earnings-price is the earnings-to-price ratio, which is the difference between the log of earnings and the log of prices. The Dividend-Price ratio is the difference between log of dividends and log of prices. \\

The below figure displays the number of firms per industry represented in the dataset according to the Standard Industrial Classification (2-digit SIC). The figure only includes industries with +50 companies, as below values are deemed less relevant.
```{r} 
...
The below figures are illustrations of the macroeconomic variables included in the dataset. KOMMENTERING

###OLIVER OPINION: LAD OS KOMMENTERE PÅ DET VI SER MED ET PAR SÆTNINGER HER###
###DESUDEN SKAL barplot ikke vises her nede, og vi skal have rykket headeren op i ovenstående snippet###
