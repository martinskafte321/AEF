...
Secondly, we consider a Random Forest (RF) model. A random forest creates many regression trees and average the predictions of the trees. A regression tree is designed to find groups of observations that behave similarly, and the tree grows further into branches, bunching data on predictor variables. A single regression tree suffers from high variance and potential overfitting, however, a Random Forest model addresses these shortcomings. In the model, we tune hyperparameters deciding the number of regression trees, trees, and the minimum amount of observations in each node, min_n. \
...
en <- autoplot(lm_tune, metric = "rmse") +
  labs(title = "RMSE for various Elastic Net and Random Forest models")
