...
rf_tune <- rf_fit %>%
  tune_grid(
    resample = data_folds,
    grid = grid_regular( min_n(), levels =  3),   # Tuning
    metric = "rmse"
  )
...
The below figure illustrates the effects of different hyperparameters on the predictive performance of the models. The figure shows the development of the cross-validated prediction errors as the amount of regularization (penalization) increases. As visible in the figure, all three models show a drop in the prediction errors as the regularization increases, however, this jumps back up after a certain point. Moreover, the errors of both the Elastic Net and Lasso estimates decreases with less regularization relative to the Ridge regression. We recall that the models become more restricted as the regularization increases. Thus, we aim to choose the model which exhibits the lowest RMSE while limiting the amount of regularization. In this case, we therefore would argue for the Lasso model (blue) as it shows a faster decrease in the MSPE than the elastic net while hitting its minimum at a lower level of regularization than the Ridge model.


```{r fig.align = 'center', fig.width=8, fig.height=4}
# Illustrations of effects of different hyperparameters on predictive performance. 

autoplot(lm_tune, metric = "rmse") +
 ...
 autoplot(rf_tune, , metric = "rmse") +
  labs(y = "Root mean-squared prediction error",
       title = "RMSE for various minimal node sizes in Random Forest")
...
```
After tuning the Elastic Net (EN) and Random Forest (RF), we can extract the best performing hyperparameter combinations from the data tested on the validation set. The two best performing models are very close when comparing the rmse of the validation sample. The RF has a rmse of 0.155, while the EN-model has a loss of 0.165. 
...
pred_collected_validation %>%
  ...
  arrange(rmse) %>% 
  kbl(caption = "Validation set prediction error", digits = 3) %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
rf_tune %>%
  ...
  kbl(caption = "Top 2 hyperparameter combinations for Neural Network", digits = 3) %>%
  kable_classic(full_width = F, html_font = "Cambria")
 
```
We have created two different models by the Random Forest method. Due to restrictions on computation power it was only possible to tune one hyperparameter on three levels. The minimal amount of nodes per three was tuned, and as the table above shows, the best performing model had 21 "min_n". RMSE of 0.279 was very close to the model of 40 min_n (rmese = 0.28), and far from the one with 2 (0.299), thus assessing that the larger the value, the better the model in this case.  \\
...
```
For the Elastic Net both penalty rate and mixture was tuned. The best model has a penalty of 1 and mixture of 0. It's difficult to conclude on this, since the other good models have dissimilar penalty rates and mixtures meaning that no level in general is preferable. 
