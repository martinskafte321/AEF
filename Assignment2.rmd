...
data_folds <- vfold_cv(training(newsplit), v = 2)
```
...
rf_tune <- rf_fit %>% # We mistakenly name the neural net = lm_tune, even though this is the name we use for the linear regression
  tune_grid(
    resample = data_folds,
    grid = grid_regular(min_n(), levels = 2),   # Tuning
    metric = "rmse"
  )
...
The below figure illustrates the effects of different hyperparameters on the predictive performance of the models. The figure shows the development of the cross-validated prediction errors as the amount of regularization increases. As visible in the figure, all three models show a drop in the prediction errors as the regularization increases, however, this jumps back up after a certain point. Moreover, the errors of both the Elastic Net and Lasso estimates decreases with less regularization relative to the Ridge regression. We recall that the models become more restricted as the regularization increases. Thus, we aim to choose the model which exhibits the lowest MSPE while limiting the amount of regularization. In this case, we therefore would argue for the Lasso model (blue) as it shows a faster decrease in the MSPE than the elastic net while hitting its minimum at a lower level of regularization than the Ridge model.
...
rf_tune %>%
  ...
  add_column('N trees' = 15, .before = 'rmse') %>%
