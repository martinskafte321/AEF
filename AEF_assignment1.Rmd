...
abs_return_plot <- returns %>%
  ...
  labs(
    ...
    title = "Average absolute excess return"
  )

acf_return <- returns %>%
  select(ret_excess) %>% 
  forecast::ggAcf() + labs(title = "ACF of excess returns")

acf_abs_return <- returns %>%
  transmute(ret_excess = abs(ret_excess)) %>%
  select(ret_excess) %>% 
  forecast::ggAcf() + labs(title = "ACF of absolute excess returns")

ggarrange(return_plot, acf_return, abs_return_plot, acf_abs_return, ncol = 2, nrow = 2)

```
As the mean of returns is non-constant, it does not fluctuate around a constant value, returns suffer from autocorrelation. Furthermore, there is a negative correlation between $r_t$ and $r_{t-1}$. The PACF plot clearly shows that there is a significant relationship between $r_t$ and $r_{t-1}$. 

We make sure to investigate autocorrelation in excess returns by looking at time-series of specific firms, and not the averages from CRSP. 
...
# Summarise the previously created monthly summary statistics into one row by taking the mean in each column
mom_stat %>% 
...
The value of momentum in the average month is 18.99% and the cross-sectional standard deviation is 95.41%. In other words, the high mean momentum fluctuates significantly. 
...
The plot shows the new portfolios sorts that have been created. Portfolio 1 has the lowest momentum and 10 has the highest. 
...
# 5. 
**Repeat the univariate portfolio sorts but this time using the k-month-ahead excess return as the outcome variable for values of k ∈{1, 3, 6, 12}. For which time-horizon does the momentum deliver the highest risk-adjusted performance?**
