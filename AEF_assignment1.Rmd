...
ret = crsp_monthly %>% 
  ...
  rename(mean=ret_excess_daily_mean, sd=ret_excess_daily_sd, min=ret_excess_daily_min, max=ret_excess_daily_max, q25=ret_excess_q25, q75=ret_excess_q75, median=ret_excess_median) %>%
  pivot_longer(
    everything()
  ) %>% 
...
ret %>% 
  cbind(mkt) %>%
  select(-n) %>% kbl(caption = "Summary statistics for monthly excess returns and market capitalization",digits = 2) %>%
...
summarymkt <- crsp_monthly %>%
  ...
  summarise(across(mktcap,
                  list(mean = mean, sd = sd, min = min,
                    q25 = ~quantile(., 0.25),
                    median = median,
                    q75 = ~quantile(., 0.75), max = max), .names = "{.fn}")) %>%
  summarise(across(-month, mean)) %>% add_column('Variable' = "mktcap")


#Combine statistics into one tibble 
table1 <- rbind(summaryret, summarymkt) %>% relocate(Variable) %>% kbl(caption = "Summary statistics for monthly excess returns and market capitalization",digits = 2) %>%
  kable_classic(full_width = F, html_font = "Cambria")
```
...
**Perform univariate portfolio sorts in the following way: Compute monthly portfolio breakpoints. Each month, use $Mom_{i,t}$ as a sorting variable. The portfolio breakpoints are the deciles of $Mom_{i,t}$ calculated using all stocks i in the sample during month t.**
