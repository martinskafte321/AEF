...
data %>% kableExtra::kable()
remove(k,i,estimate,data, lag_excess_ret)
```
...
all_guys_old <- momentum_portfolios %>%
...
all_guys_new <- momentum_portfolios %>%
  left_join(lagged_momentum, c("permno","month")) %>%
  drop_na() %>%
  mutate(portfolio = as.numeric(portfolio), portfolio_lag = as.numeric(portfolio_lag)) %>%
  group_by(month) %>%
