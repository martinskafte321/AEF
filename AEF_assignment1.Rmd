...
The table below shows that the portfolio, which doesn't short the lowest decile ends up with the highest sharpe ratio of 0.15 with the lowest turnover at 24.7\%. The portfolio, which go short the lowest decile ends up with a sharpe ratio of 0.08 and a turnover ratio of 25\%. To sum up, the best strategy seems to be without a short strategy. 
...
table = matrix(c(sharpe_old,sharpe_new,turnover_org,turnover_new), ncol=2)
rownames(table) <- c('Original portfolio','No short-portfolio')
colnames(table) <- c('Sharpe Ratio','Turnover ratio')

table %>% kbl(caption = "Sharpe ratio and turnover of portfolios", digits = 2) %>% 
  kable_classic(full_width = F, html_font = "Cambria")
