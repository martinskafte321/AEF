...
momentum = data_for_sorting %>%
  ...
  mutate(
    ...
    data = cur_data(), #use current set, i.e. data_for_sorting
      var = momentum,
      n_portfolios = 10
    ),
    portfolio = as.factor(portfolio))
...
trades_old <- momentum %>%
  ...
  group_by(month) %>%
  summarise(count_traded = sum(traded == 1)) #summarizing total monthly transactions - we dont multiply by 2 because we assume just one trade per transaction, i.e. no stock goes from pf 10 to 1 or from 1 to 10 in one month
...
trades_new <- momentum %>%
  ...
  mutate(traded = ifelse(portfolio==portfolio_lag, 0, 1)) %>% #counts 1 every time a stock moves between pf's 
  group_by(month) %>%
  summarise(count_traded = sum(traded == 1)) #summarizing total monthly transactions
```
...
sharpe_old = momentum_portfolios %>% 
  mutate(portfolio = as.numeric(portfolio)) %>%
  group_by(month) %>%
  mutate(breakpoint_low = 1,
         breakpoint_high = 10,
         portfolio = case_when(portfolio <= breakpoint_low ~ "low",
                               portfolio >= breakpoint_high ~ "high")) %>%
  filter(str_detect(portfolio, 'high|low')) %>% #removes all NA rows, i.e. observations that are not part of the portfolio
  group_by(month) %>% 
  mutate(ret_excess = if (portfolio == 'low'){
    ret_excess * (-1)
  } else ret_excess) %>% #flips the sign of excess return for all stocks subject to short-selling
...
sharpe_new = momentum %>%
  ...
  filter(portfolio=='high') %>% #removes all observations that are not part of the portfolio
  group_by(month) %>%
  summarise(excess_ret = weighted.mean(ret_excess, mktcap_lag), sd = sd(ret_excess), .groups = "drop") %>%
  mutate(sharpe = excess_ret/sd) %>%
